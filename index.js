...
const COLS = {
  ...
  "Riviera": {
    bg_prompt: "Marseille Vallon des Auffes authentic fishing port, real weathered painted boats, worn ochre peeling facades, genuine Mediterranean midday light, south of France, imperfect real photography",
    bg_negative: "people, text, logo, fake, CGI, too perfect, studio, tourist postcard",
...
<div class="app">
  ...
  <div id="ts" style="display:none">
    ...
    <div class="sb">
      <label class="fl">URL mannequin par défaut</label>
      <input type="text" id="s-model" placeholder="https://i.ibb.co/... lien direct de ta photo">
      <p class="hint">Ta photo hébergée sur imgbb — pré-remplie à chaque session</p>
    </div>
  ...
  <div id="tp">
    ...
    <div class="field">
      ...
      <p class="hint">Tu peux sauvegarder ton URL par défaut dans Paramètres</p>
    </div>
    ...
    <div id="tr" style="display:none;margin-bottom:16px">
      ...
      <img id="ti" class="ri" onclick="window.open(this.src,'_blank')">
    ...
    <div id="fr" class="divt" style="display:none">
      ...
      <img id="fi2" class="ri" onclick="window.open(this.src,'_blank')">
...
function init() {
  ...
  if (model) document.getElementById('s-model').value = model;
  if (model) document.getElementById('mu').value = model;
  ...
  document.getElementById('s-model').addEventListener('input', e => {
...
function handleFile(f) {
  ...
  dz.innerHTML = '<img src="' + URL.createObjectURL(f) + '" style="width:100%;max-height:220px;object-fit:contain;display:block">';
...
async function callProxy(ep, fields) {
  ...
  const txt = await r.text();
  if (!r.ok) throw new Error('Erreur ' + ep + ' (' + r.status + '): ' + txt.slice(0, 300));
  ...
  } catch { throw new Error('Réponse: ' + t.slice(0, 300)); }
}
...
async function run() {
  ...
  if (!modelUrl) return showErr('URL mannequin manquante — renseigne-la dans Paramètres ou dans le champ.');
  ...
  try {
    ...
    setStep(2);
    addLog('🌿 Décor ' + col + '…');
    const bgPrompt = c.bg_prompt + '. ' + c.bg_harmony;
    const finalUrl = await callProxy('change-background', { api_key: tnbKey, image: tryonUrl, replace: bgPrompt, negative: c.bg_negative });
    if (!finalUrl) throw new Error('URL vide du décor.');
    addLog('✅ Image finale prête');
    ...
    btn.disabled = false;
    document.getElementById('btnr').textContent = 'Lancer → ' + col;
  }
}
